...
const Home = () => {
  ...
  const uploadImage = async (imageUri) => {
    ...
    formData.append('crop', selectedCrop) // Add selected crop to the form data
    ...
    try {
      ...
      const sortedProbabilities = Object.entries(probabilities)
        .filter(([key]) => key.includes(selectedCrop)) // Filter probabilities by selected crop
        .sort(([, a], [, b]) => parseFloat(b) - parseFloat(a))
        .slice(0, 3)
        .reduce((acc, [key, value]) => {
      ...
      const total = Object.values(sortedProbabilities).reduce(
        (acc, val) => acc + val,
        0
      )
      const normalizedProbabilities = Object.entries(
        sortedProbabilities
      ).reduce((acc, [key, value]) => {
      ...
      setResult({
        predictedClass,
        topProbabilities: normalizedProbabilities,
      })
    } catch (error) {
      if (error.response) {
        console.error('Server response error:', error.response.data)
